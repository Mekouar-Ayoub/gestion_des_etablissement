...
function LoginEleve() {
  ...
  var handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        process.env.REACT_APP_API_URL + "/eleve/login",
        {
          email,
          password,
        }
  ...
  return (
    <div
      className="flex bg-blue-symphony h-[100vh]"
    >
      ...
      <div className="h-[100%] mt-[5%] border-2 mr-[10%] ml-[10%] student-border">
        <div className="flex justify-center mt-[5%] ">
          <Link to="/eleve/login">
            <RoundedButton
              text={"Eleve"}
              type="login"
              className="student-background border-2 border-solid text-white"
            ></RoundedButton>
          </Link>
          <Link to="/prof/login">
            <a href="/prof/login">
              <RoundedButton
                text={"Enseignant"}
                type="login"
                className={"border-2 hover:bg-blue-400 border-solid hover:text-white"}
              ></RoundedButton>
            </a>
          </Link>
        </div>
        ...
        <div className=" mt-[10%]">
          ...
          <div className="ml-[15%] mt-[3%]">
            <Checkbox></Checkbox>
            <span className="text-stone-600 text-sm mt-[15%]">
              Restez connécté
            </span>
          </div>
          {errorMessage && (
